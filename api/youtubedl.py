#!/usr/bin/env python3
"""
YouTube Video & Channel Info Extractor with Download Links
Usage: python yt.py <youtube_url>
Example: python yt.py https://youtu.be/sRWcJrMTtMI?si=uFycx1rbsQehD1AR
"""
...
def extract_channel_id(text):
    """Extract channel ID from URL or HTML content"""
    patterns = [
...
def extract_handle_from_html(text):
    """Extract @handle from HTML - ONLY use on channel pages, not video pages"""
    patterns = [
...
def format_count(num):
    """Format large numbers (views, likes, subs)"""
    if not num:
        return None
    try:
        n = int(num)
...
def parse_keywords(keywords_str):
    """Parse channel keywords string, keeping quoted phrases together"""
    if not keywords_str:
        return []
    keywords = []
    current = ""
...
def get_video_info(video_id, video_url):
    ...
    # Calculate human-readable fields
    duration_formatted = None
    if info.get('duration_seconds'):
        duration_formatted = format_duration(info['duration_seconds'])

    view_count_formatted = None
    if info.get('view_count'):
        view_count_formatted = format_count(info['view_count'])

    like_count_formatted = None
    if info.get('like_count'):
        like_count_formatted = format_count(info['like_count'])

    # Reorder: formatted fields right below view_count
    ordered_info = {}
    inserted = set()
...
def main():
    if len(sys.argv) < 2:
        ...
        print("Example: python yt.py https://www.youtube.com/watch?v=bMoNV9oWs2E")
        print("")
    ...
    # print(f"Fetching info for video ID: {video_id}...", file=sys.stderr)

    # Get video info
    video_info, channel_id, channel_handle = get_video_info(video_id, url)

    # Get download info from API
    # print("Fetching download links from API...", file=sys.stderr)
    ...
    # Build final response - exact format requested
    result = {}

    # Clean video_details: remove None values but keep empty strings and False
    cleaned_video = {}
